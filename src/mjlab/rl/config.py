...
class RslRlModelCfg:
  ...
  """Distribution config dict passed to rsl_rl.
...
class RslRlPpoAlgorithmCfg:
  ...
  Otherwise, the advantage is normalized over the entire collected
  trajectories.
  """
...
class RslRlBaseRunnerCfg:
  """Base configuration shared by RSL-RL runners."""
  ...
  """Observation groups used by the actor and critic.

  Multi-critic runners override this automatically based on
  ``algorithm.num_critics``.
  """
  ...
  """The clipping range for action values.

  If None, no clipping is applied.
  """

  upload_model: bool = True
  """Whether to upload model files to W&B on save."""


@dataclass
class RslRlOnPolicyRunnerCfg(RslRlBaseRunnerCfg):
  """Configuration for the standard single-critic PPO runner."""
...
@dataclass
class RslRlMultiCriticOnPolicyRunnerCfg(RslRlOnPolicyRunnerCfg):
  """Configuration for MultiCriticPPO.

  The ``critic`` configuration is used as a template for all critics.

  The number of actual critic networks is controlled by
  ``algorithm.num_critics``.
  ...
  The environment must expose matching observation groups:
  ...
  class_name: str = "MultiCriticOnPolicyRunner"
  """The runner class name."""
  ...
  """Configuration template shared by all critic networks.

  The actual number of critics is controlled by
  ``algorithm.num_critics``.
  """
  ...
  def __post_init__(self) -> None:
    """Generate observation groups for all critics."""

    self.obs_groups = {
      "actor": ("actor",),
      **{
        f"critic_{i}": (f"critic_{i}",)
        for i in range(self.algorithm.num_critics)
      },
    }